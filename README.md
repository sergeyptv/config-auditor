...
| FS001 | MEDIUM | Файл доступен для записи группе |
| FS002 | HIGH | Файл доступен для записи всем пользователям |
| FS003 | LOW | Файл доступен для чтения группе |
| FS004 | MEDIUM | Файл доступен для чтения всем пользователям |
...
-r, --recursive
    Рекурсивно проверить JSON и YAML файлы в указанной директории.
```
...
Проверка всех поддерживаемых конфигураций в директории:

```bash
./bin/config-auditor --recursive ./configs
```

Короткий вариант:

```bash
./bin/config-auditor -r ./configs
```

Поддерживаются файлы с расширениями:

- `.json`
- `.yaml`
- `.yml`

Другие файлы и символические ссылки пропускаются.

Каждая проблема содержит путь к исходному файлу:

```bash
HIGH [CFG002] configs/production.yaml:database.password
```

Если один из файлов невозможно прочитать или разобрать, утилита продолжит
проверять остальные конфигурации, выведет обнаруженные проблемы и завершится
с кодом `2`.
...
Проверка файловых прав выполняется только при передаче пути к файлу.
При использовании `--stdin` у входного потока может отсутствовать файловый путь, поэтому права доступа не анализируются.

Проверка основана на Unix-битах доступа и предназначена в первую очередь для Linux и macOS.